	...
	public void validateUsersWithMoreThan50PercentCompletedTasks() {
		...
		for (Users user : users) {
			if (user.getAddress() != null && user.getAddress().getGeo() != null) {
				System.out.println("Geo data for user: " + user.getName() + " | Lat: "
						+ user.getAddress().getGeo().getLat() + " | Long: " + user.getAddress().getGeo().getLng());
			} else {
				System.out.println("Geo data missing for user: " + user.getName());
			}
		}
		// Filtering users by 'FanCode' city based on latitude and longitude
		...
		// Validating each user's completed task percentage
